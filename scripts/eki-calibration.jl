...
p1a = plot_bwfl_data(df, ylabel, ymax=0.8)
...
n_train = (4 + 1) * 24 * 4  # 4 train days (day 1 discarded in wq simulation)
...
p1b = plot_bwfl_data(df_filter, ylabel, ymax=0.8)
...
begin
    ...
    function run_eki_calibration(θ_b, θ_w_lb, θ_w_ub, wn, datetime, exclude_sensors, grouping, ȳ; burn_in=24*4, δ_s=0.1, δ_b=0.025, wall_prior="uniform", n_ensemble=100, n_iter=10)
        ...
        for i in 1:n_iter
            ...
            g_ens_full = hcat([forward_model(wn, θ_i[:, j], grouping, datetime, exclude_sensors; sim_type="chlorine", burn_in=burn_in) for j in 1:n_ensemble]...)
            g_ens_valid = g_ens_full[valid_indices, :]
            ...
            for j in 1:n_ensemble
                valid_residual_j = g_ens_valid[:, j] - ȳ_valid
                temp_abs_residuals_full_size = fill(NaN, length(ȳ))
                temp_abs_residuals_full_size[valid_indices] = abs.(valid_residual_j)
                reshaped_abs_residuals_for_mean = reshape(temp_abs_residuals_full_size, n_time, n_sensor)
                mean_residuals[j, :] = mean(reshaped_abs_residuals_for_mean, dims=1)
                append!(loss, dot(valid_residual_j, Γ_valid \ valid_residual_j))
            end
            ...
            # update ensemble
            status = EKP.update_ensemble!(process, g_ens_valid)
            if status == true
                k += 1
                if k > 1
