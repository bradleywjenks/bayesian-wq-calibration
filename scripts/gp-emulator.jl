...
the following steps are performed:
    ...
    4. get test data for validation (beyond the EKI ensemble)
    5. validate gp model performance
    6. visualize results
"""
...
if !isempty(test_idx)
    p2 = plot_time_series(y_test, y_pred, y_std)
end

p3 = plot_error_histogram(y_test, y_pred)
display(p3)

plots_output_path = RESULTS_PATH * "/wq/gp_emulator/plots/"
save_plots(p1, p2, p3, plots_output_path)
...
function save_plots(p1, p2, p3, output_path)
    mkpath(output_path)
    
    savefig(p1, joinpath(output_path, "$(data_period)_$(selected_sensor)_$(grouping)_parity.png"))
    savefig(p2, joinpath(output_path, "$(data_period)_$(selected_sensor)_$(grouping)_timeseries.png"))
    savefig(p3, joinpath(output_path, "$(data_period)_$(selected_sensor)_$(grouping)_error_hist.png"))
